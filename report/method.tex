...
The $f$ parameter can be anything that returns a probability. We are going to
present several different networks that can be used as $f$ as they will take two
texts and end with a softmax layer that outputs a probability distribution.

The $w$ parameter in the prediction system can be used to weigh the known
texts differently. For example we could define $w$ such that the newest
text is weighed as most important and older texts are weighed less and less
important. $w$ could be defined as $w(t_1), w(t_2), \dots, w(t_n) = \frac{1}{2},
\frac{1}{4}, \dots, \frac{1}{2^{n-1}}, \frac{1}{2^{n-1}}$ where $t_1$ is the
newest text, $t_2$ is the second newest and so on. Otherwise $w$ can also
be used to take a simple average of the decisions by defining it as $w(t) =
\frac{1}{|T(\alpha)|}$.

The $\theta$ parameter determines when we consider an unknown text to be written
by an author. The natural choice for $\theta$ is 0.5 since that is what the
network use when it computes the loss of each training sample. However the
$\theta$ parameter can be used to enforce how sure we have to be of a decision
to accuse an author of not having written an assignment. As described earlier
MaCom does not want to accuse innocent students of cheating. That means that
it is very important to MaCom to minimize the number of \gls{FN}'s compared
to the total number of accusations raised. That means that we have to keep
$\frac{FN}{FN + TN}$ below a threshold chosen by MaCom. We can use the $\theta$
parameter to control that fraction. Hopefully the \gls{FN}'s generally end up
with a value closer to 1 than the \gls{TN}'s. If that is the case then lowering
$\theta$ value will lower the fraction since there will be fewer \gls{FN}'s.
That might lower the overall accuracy of the prediction system but will make
sure that at few students as possible are falsely accused.
