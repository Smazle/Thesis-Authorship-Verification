...
trained neural network $N$ and an author $\alpha$ with $T(\alpha)$ previous
texts we will verify whether or not $x$ is written by $\alpha$ by using the
network to compare each $t \in T(\alpha)$ to $x$. That gives us $|T(\alpha)|$
probabilities $p\in P$ of the text $x$ being written by the same author as that $t
\in T(\alpha)$. We then take a weighted average over $P$ to obtain a final
probability $p$. We then threshold that final probability such that we don't
accuse to many people of cheating.
