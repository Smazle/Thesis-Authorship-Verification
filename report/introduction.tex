...
as information retrieval, machine learning and \gls{NLP}.\cite{stamatos2009}
...
texts and authors. We will generally name texts $t$ and authors $\alpha$.
If multiple texts or authors are used we will generally call them $t'$ and
$\alpha'$ or $t_n$ and $\alpha_n$ if many texts or authors are needed.
The set $\mathcal{A} = \{\alpha_1, \alpha_2, \dots, \alpha_n\}$ denotes
all authors, where $T_\alpha$ denotes the set of texts written by the author
...
Texts will be represented by a sequence of characters. We denote the length
...
sequences of \textit{object-n-grams} \citep{stamatos2009}. The object could for
example be characters and the n in n-gram would then refer to the length of
character sequences in a text. If we take the text to be "hello" the different
character-3-grams will be ``hel'', ``ell'' and ``llo''. The frequency of each of them
is $\frac{1}{3}$, and would service as the input for the specific model. We will
use n-grams in some of our baseline methods and some of our networks will use
them indirectly.

% Introduce the main ideas that stem from your topic/title and the order in
% which you will discuss them?

As described the problem we try to solve in this thesis is
...
In Section \ref{sec:related_work} we will go through previous work on authorship
...
In Section \ref{sec:results} we present our testset performances for our baseline,
