...
\begin{description}
    ...
    \item[$P_{exp_\lambda}$]
        ...
        The exponential dropoff weight function used the turn in time of an
        assignment to determine the most important text. Recall that as $\lambda
        \rightarrow \infty$ the more weight is placed on the newest assignment.
        We assumed that an authors writing style would change over time and the
        ...
        tried we observe that at the extremes of the graphs (near 0 and 1)
        better accuracy is obtained as $\gamma$ increases. But similarly we
        have a higher accusation error as $\gamma$ increases. However at the
        peak of the graph better accuracy is obtained using the lowest $\gamma$
        value and the best accusation error is also obtained using the lowest
        $\gamma$ value. We have illustrated the effect in Figure \ref{fig:todo}.
