...
showed in \ref{subsubsec:conv_char_word_nn}, even \gls{CNN}s performed badly
on the sentence level. Ideally we would have run an RNN on the word level. We
suspect it would be able to learn the words and their relation to on another,
which makes sense with words, but not with sentences. However in order to do so,
we would need computational power than was available to train the model. Looking
at the performance of the two \glspl{CNN}, it becomes obvious that the character
...
\subsubsection{Comparison with previous MaCom works}

As mentioned all the way back in Section
\ref{subsec:previous_work_using_macoms_dataset} other papers have been produced
with Macoms data as their basis. Much like us \cite{hanse2014}, made use
of an SVM classifier, but for authorship attribution rather and authorship
verification. When all was said and done, they got an accuracy of 84\%. This
means that not only did they beat our SVM implementation, but our neural
networks as well. We do however think we can justify this difference. While they
did have access to the same large dataset we did, they chose only to extract
the library of 30 authors. Due to the author-specific nature of their SVM
implementation, each of those 30 authors texts got further split into training
validation and test data. This approach could very well result in a giant bias
towards those specific authors. For their purposes it might have made sense, as
they had to remake their attribution task, to a one-against-many verification
task. When considering that, and the fact that most likely had a better set of
negative samples to train on, the results are understandable.

\cite{aalykke2016} also worked on MaComs data, however using a set of distance
measure and thesholding. They got an impressive 71.9\% \gls{TNR} on their negative
cases, while adhering to a 5\% accusation error constraint.
