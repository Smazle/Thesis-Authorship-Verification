...
infeasibility of running any of our \glspl{RNN} on the character level of the
texts supplied to it. We hypothesize that the accuracy of a RNN would be greatly
increased had it been run on a lower linguistic level. As experiments showed in
\ref{subsec:conv_char_word_nn}, even \gls{CNN}s performed badly on the sentence
level. Ideally we would have run and RNN on the word level. We suspect it
would be able to learn the words and their relation to on another, which makes
sense with words, but not with sentences. However in order to do so, we would
need computational power than was available to train the model. Looking at
