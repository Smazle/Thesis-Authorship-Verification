...
methods, and why their final structure and parameters are what they are. Not only that, but each method will be tested against the external validation set
...
accuracy of 0.697 and 0.672 respectively.

\begin{figure}
    \centering
    \textbf{Accuracy of baseline methods during feature selection}
    \includegraphics[scale=0.8]{./pictures/experiments/feature_selection}
...
error function we used the binary cross-entropy function described whose definition
can be seen in Equation \ref{eq:binary_ce}.

\begin{definition}[Binary Cross-Entropy]
    The loss function Binary Cross entropy is defined as such

    \begin{equation}\label{eq:binary_ce}
    L(y, p) = -(y \cdot \log(p) + (1- y)\cdot\log(1 - p))
    \end{equation}

    Where y is a label $y : \{0,1\}$ and p is a predicted probability $p \in
    [0,1]$.

\end{definition}


 When we trained the network we were able to obtain a validation
...
10 epochs this network peaked with a validation accuracy of 0.657. 
...
\item[Embedding:]
    ...
    \item[Decision:]
        ...
        function used for the network was binary cross-entropy, paired with
