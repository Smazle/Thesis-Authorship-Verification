...
\ref{subsubsec:applying-other-machine-learning-methods}).

This is not the case for both the \gls{SVM} and the extended delta method that
does not perform any kind of quality analysis of the features they are given.
It is worst for the extended delta method as that is just a distance metric
that has no way of weighing the features. An \gls{SVM} finds the best linearly
separating hyperplane in some dimension by using support vectors and the kernel
trick. The inclusion of non-descriptive features could potentially decrease the
quality of the found hyperplane as they could cause some samples to be misplaced
thus making finding a good linear separation harder. However using the $C$ and
$\gamma$ parameter this can be combated as $\gamma$ denote samples influence
relative to distance, and $C$ denotes the tradeoff between miss-classification
and a smooth class-separator. Therefore we chose to perform a feature selection
for them rather than just having them use the entire set available to them.
Contrary to what was done by \citet{US} this process did not consist of trying
out some random selection of features but rather a more systematic approach was
used.
