...
\begin{equation}
    ...
    \begin{aligned}
        ...
        \right| \\
        & \text{subject to}
        & & \sum_{(\alpha, t_u) \in V} \mathbbm{1}_{T_\alpha}(t_u) \cdot
            \left(1 - P(f, w, T_\alpha \setminus \{t_u\}, t_u, \theta)\right) <
            \frac{1}{10} \sum_{(\alpha, t_u) \in V}
                 (1 - P(f, w, T_\alpha \setminus \{t_u\}, t_u, \theta)
    \end{aligned}
\end{equation}
...
as, $\textit{false accusations} < \frac{1}{10} \textit{total accusations}$.
Consider the value of the left hand side,
...
\ref{fig:prediction_system_results_1}.
...
\begin{figure}
    ...
    \caption{Results of running the prediction system with our best
        convolutional network. In the left graph we show the accuracies
