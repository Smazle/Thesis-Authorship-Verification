...
\begin{center}
\begin{verbatim}
Extended Delta Validation Accuracy: 0.586
SVM Validation Accuracy:  0.680
\end{verbatim}
\end{center}
...
previous experiments made described in \citet{qian:2018}, which showed real
...
the reason why a recurrent network might not be the best choice for a task
like this. The validation error throughout training the network was not very
high compared to earlier convolutional network, and after running it through
the prediction system it became obvious that the network was really good a
learning individual author. In a task such as this, were generalization is a
