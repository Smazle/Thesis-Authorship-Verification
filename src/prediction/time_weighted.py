...
import jsonpickle
import random
# TODO: Newest text should be the one we use test against.
...
with LineReader(args.datafile, encoding='utf-8') as linereader:
    for i, (unknown, knowns, label) in enumerate(problems):
        ...
        # fractions = map(lambda x: 1/x, range(b))
        fractions = list(map(lambda x: 1/(2**x), range(1, len(predictions))))
        fractions.append(fractions[-1])

        assert len(fractions) == len(predictions)
        final_prediction = np.sum(np.multiply(np.array(fractions), np.array(predictions)))
